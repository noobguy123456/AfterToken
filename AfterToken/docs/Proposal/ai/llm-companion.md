...
> 提案状态：M1~M6 全部落地并在线实测通过（2026-09-07/08，DeepSeek，见 `docs/modules/ai/companion-system/progress.md`）；另完成主动开火（警戒圈先发制人）与人设强化
> 提出时间：2026-09-07
...
### 6.5 人格稳定策略（2026-09-08 补充：为什么不用 LoRA）

**为什么 LoRA 搁置**：LoRA 适合"输出长、格式自由、风格微妙"的场景；Rook 的输出 ≤60 字、格式被契约占掉大半，且**玩家没有自由文本输入**（所有输入都是代码拼的结构化快报），LLM 人格最大的破坏者（用户输入带偏/越狱）在架构上不存在。prompt 路线（人设卡+few-shot）对此长度可拿到八九成效果，改动成本是改表 vs 重训。未来若有长段剧情输出需求，经 M5 的官方托管通道换 endpoint 即可引入微调模型，客户端零改动。

**纯 prompt 人格稳定五层防线**：

1. **身份锚**：personaPrompt 写细（背景/口癖/禁忌/关系定位），每次请求重新注入。
2. **语气锚**：system prompt 内置 8~12 条 few-shot 示例（输入格式与运行时完全一致），锁死语气/长度/措辞。
3. **无状态调用**：每次请求独立（人设+示例+全新快照），不存在长对话漂移——第 100 句与第 1 句人格起点相同。
4. **代码兜底**：契约解析失败/超长 → 丢弃转本地 bark，LLM 最差输出不到达玩家；temperature 0.3~0.5。
5. **回归测试**：20 个固定战场快照"考题"，改人设/换模型后跑一遍，出契约合规率/超长率数据 + 人工抽评语气。
6. **版本钉死**：配置的 model 字段写具体版本号（如 `kimi-k2-0905-preview`），防服务商后台升级导致隐形漂移。

**业界参照**：《历史模拟器：崇祯》为纯 prompt 路线（千问基座 + 每回合状态注入，无微调），制作人明确"输入的不是史料而是当前状态"；其最大教训是 token 成本失控——BYOK 模式下成本由玩家承担，但请求限额/对话频率限制仍是必备设计。

**要不要 LoRA 的判断标准**（2026-09-08 与用户确认）：LoRA 是 prompt 实测失败后的逃生舱，不是起点。满足以下全部条件即不需要 LoRA——① 单次输出短（≤3 句）；② 有固定输出契约；③ 角色知识一张人设卡写得完；④ 人设需频繁调整；⑤ BYOK 模式。唯一触发信号：人设卡写到几千字 + few-shot 塞满后回归测试语气达标率仍不达标。届时走 M5 官方托管通道引入微调模型，与 BYOK 并存。
...
### 技术路线决策（2026-09-08）：BYOK 高拟人路线，LoRA 搁置

用户核心诉求：**小包体 + 玩家自带 API key + 高拟人队友**。

- **LoRA 与 BYOK 互斥**：LoRA 权重必须由推理服务在服务端加载，玩家个人 API key 无法让第三方端点加载我们的 LoRA。因此 LoRA 训练/自部署推理服务/端侧小模型三条路线全部**搁置**（评估记录见 2026-09-08 讨论：LoRA 训练链路 LLaMA-Factory+Qwen2.5 基座、vLLM 部署、端侧 llama.cpp 均评估过，与诉求不符）。
- **主线 = Prompt 工程纵深**：拟人度靠人设卡细节化、few-shot 示例、记忆系统（跨局长期记忆本地存储）、情绪状态机反馈进 prompt 来实现——这些全部在 prompt 层，玩家任何 OpenAI 兼容端点都能享受，包体零增长。
- 客户端架构天然兼容：`LlmClient` endpoint 可配，未来若推出"官方托管通道"（自部署 LoRA 服务）只是换一个默认 endpoint，玩家自选 BYOK 或官方通道。
...
| **M5 实时操控通道**（后加） | DecisionDriver 决策节拍 + 战场快报 + 8 动作契约 + 遥测 | LLM 实时驱动队友动作，合法率/延迟可观测 |
| **M6 自由对话**（后加） | T 键聊天 UI + 多轮记忆 + 玩家指令 intent 裁决 | 安全态自由对话、战斗中拒聊、对话指令改变姿态 |

M1/M2 不依赖任何网络，可先完整落地；M3~M6 是增量。
