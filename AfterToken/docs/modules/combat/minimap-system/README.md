...
战斗场景右上角的小地图，采用**传统 2D 平面缩略图**样式（参考 Apex / 三角洲行动）：入场时对整张地图做一次正交俯视**一次性烘焙**成静态纹理，之后相机停用、不再逐帧渲染 3D 场景；UI 通过 `RawImage.uvRect` 平移一个以玩家为中心的观察窗口来显示局部地图，窗口钳制在地图范围内，**永不显示场景外区域**。玩家与敌人以平面图标投影。
