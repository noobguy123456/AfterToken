...
|---------|----------------------|------|
| 普通全屏 HUD / 菜单 | 否 | 使用 `UIRoot/UICanvas` 的统一缩放（2026-08-18 起参考分辨率为 **1920×1080 横屏**，`matchWidthOrHeight = 0.5`；此前为 TEngine 默认竖屏 750×1334 按宽适配，曾导致 1920×1080 下整体放大 2.56 倍、硬编码像素布局溢出屏幕） |
| 需要独立设计分辨率的面板 | 是 | 如 `SniperScopeUI`、`WeaponWheelUI` 按 1920×1080 设计 |

> 注意：所有 UI 的统一设计分辨率为 **1920×1080**，代码/Prefab 中的像素尺寸与坐标均按此逻辑分辨率编写，不要再为抵消根缩放做反向缩放。
