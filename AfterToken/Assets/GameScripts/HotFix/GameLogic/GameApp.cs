...
namespace GameLogic
{
    ...
    public partial class GameApp
    {
        ...
        private static async UniTaskVoid EntranceAsync(object[] objects)
        {
            ...
            GameModule.Initialize(RootModule.Instance);
            RetainSceneRoots();
        ...
        /// <summary>
        /// 启动时将场景中已存在的根节点注册到 SingletonSystem，避免后续运行时 GameObject.Find。
        /// </summary>
        private static void RetainSceneRoots()
        {
            RetainGameObjectIfFound("UIRoot");
            RetainGameObjectIfFound("LoadUpdateUI");
            RetainGameObjectIfFound("LoadTipsUI");
        }

        private static void RetainGameObjectIfFound(string name)
        {
            var go = GameObject.Find(name);
            if (go != null)
            {
                SingletonSystem.Retain(go, null);
            }
        }
        ...
        private static void TryResumeLastProcedure()
        {
            ...
            var oldBattleRoot = SingletonSystem.GetGameObject("BattleRoot");
        ...
        private static void StartGameLogic()
        {
            ...
            procedureModule.StartProcedure<ProcedureMainMenu>();
        }
